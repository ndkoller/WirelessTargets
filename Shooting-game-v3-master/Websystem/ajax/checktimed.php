...
$stmt = $db->prepare('SELECT * FROM activetimed WHERE isdone = :isdone AND isadded = :isadded AND gplayer = :gplayer');
...
//Mark as added
try {
    $stmt = $db->prepare('UPDATE activetimed SET isadded = :isadded WHERE id = :id');
    $stmt->execute(array(
':isadded' => '1',
':id' => $roundid
));
