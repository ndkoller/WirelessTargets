...
<head>
	...
	<title><?=gettext('Loading game')?></title>
	<meta charset="utf-8">
		...
		<div class="section-content">
			<div class="container">
				...
				<div class="section-heading section-heading--divider-top">
					<h2 class="section-heading__title"><?=gettext('Loading game')?></h2>
					<h4 id="playName"><?=gettext('Please hold.. Shuffle player.')?></h4>
				</div>
							</div>
						</div>
		...
		jQuery.ajax({
            ...
            success: function(data) {
                ...
                if (fel == "true") {
					...
					if (gamemode == '1') {
                    $(location).attr('href','runquick.php')
				} else if (gamemode == '2') {
					$(location).attr('href','runtimed.php')
				} else if (gamemode == '3') {
					$(location).attr('href','runrapid.php')
				}
                }
            }
        });
	...
	setTimeout(function(){
		jQuery.ajax({
            ...
            success: function(data) {
                ...
                var kod = mydata.errorcode;
                var kod1 = mydata.error1;
                var playname = mydata.playname;
                if (fel == "true") {
                    if (kod == "1") {
                        alert('<?=gettext('Database error');?>: ' + kod1);
                    }
                } else if (fel == "false") {
                    $("#playName").html('<?=gettext('Time for player:');?> '+playname);
