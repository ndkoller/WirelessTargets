...
$stmt = $db->prepare('UPDATE settings SET sendbat = :sendover WHERE id = :id');
$stmt->execute(array(
':sendover' => '1',
':id' => '1'
));
...
<html lang="zxx">
<head>
<title><?=gettext('Checking Battery status')?></title>
	<meta charset="utf-8">
		...
		<div class="section-content">
			<div class="container">
				...
				<div class="section-heading section-heading--divider-top">
					<h2 class="section-heading__title"><?=gettext('Battery status')?></h2>
					<h4 id="playName"><?=gettext('Please hold... Checking the battery status.')?></h4>
				</div>
							</div>
						</div>
		...
		jQuery.ajax({
            ...
            success: function(data) {
                ...
                if (fel == "true") {
                    ...
                    $(location).attr('href','batstatus.php')
                }
            }
        });
}, 20000);
